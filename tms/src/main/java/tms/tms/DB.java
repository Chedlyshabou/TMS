...
public class DB
{
    public static boolean checkUser(String email, String password)                                   //Will display the list of all the customers for manager
    {
        try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + Globals.getDb_name(), Globals.getDb_username(), Globals.getDb_pass());)
        {
            if (conn != null)
            {
                String query = "select * from members";  // query to be sent
                ...
                while (rs.next())
                {
                    ...
                    System.out.println(DBemail + " " + DBpass);
                    System.out.println(email + " " + password);
                    if (DBemail.equals(email) && DBpass.equals(password) ) return true;
                }
            }
            else
            {
    ...
    public static boolean checkUser(String email)                                   //Will display the list of all the customers for manager
    {
        try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/" + Globals.getDb_name(), Globals.getDb_username(), Globals.getDb_pass());)
        {
            if (conn != null)
            {
                ...
                {
                    String DBemail = rs.getString("email");
                    if (DBemail.equals(email)) return true;
                }
            }
            else
            {
    ...
    public static void insertUser(int id, String name, String surname, Date birthday, String email, String pass)
    {
        User user = new User(name, surname, birthday, email, pass);
        try (
        ...
        {
            if (conn != null) {
                System.out.println("Database - inserting a user");
                String query = "select * from members";  // selects all data from database
                Statement stmt = conn.createStatement();
                ResultSet rs = stmt.executeQuery(query);
                String last_id = "10000";
                while (rs.next()) {                                            // reads a row
                    last_id = rs.getString("user_id");             // reads an id
                }
                int temp = Integer.parseInt(last_id);
                temp++;
                String new_id = Integer.toString(temp);
                System.out.println(birthday);
                query = "insert into users(user_id, name, surname, birthday, email, password)" +
                        "values('" + new_id + "','" + name + "','" + surname + "','" + birthday + "','"
                        + email + "','" + pass + "')";
